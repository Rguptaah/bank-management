...
function send_msg($number, $sms)
{
	...
	if (preg_match('/^[6-9]{1}[0-9]{9}+$/', $number) == 1) {
		...
		$url = "http://msg.morg.in/rest/services/send_sms/sendGroupSms?AUTH_KEY=$auth_key_msg&message=$msg&senderId=$sender_id&routeId=1&mobileNos=$no&smsContentType=english";
